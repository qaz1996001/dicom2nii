...
def list_dicom(data_path: pathlib.Path):
    ...
    df1 = df1.fillna(0)
    df1.to_excel(str(data_path.parent.joinpath(f'{data_path.name}_df_dicom.xlsx')),merge_cells=False)
    # df1.to_csv(str(data_path.parent.joinpath(f'{data_path.name}_df_dicom.csv')))
