...
from .config import ASLSEQSeriesRenameEnum, DSCSeriesRenameEnum, MRSeriesRenameEnum
    ...
    def process(self, study_path: pathlib.Path, *args, **kwargs):
        ...
        for series_folder in tqdm(
            ...
            try:
                ...
                dicom_header = {}
                dicom_list = [
                    dcmread(str(x), force=True, stop_before_pixels=True)
...
class MRDicomProcessingStrategy(ProcessingStrategy):
    ...
    def process(self, study_path: pathlib.Path, *args, **kwargs):
        ...
        for series_folder in tqdm(
            series_folder_list, desc=f"MRDicom : {study_path.name}"
        ):
            dicom_list = [
                (dcmread(str(x), force=True), x) for x in series_folder.iterdir()
            ]
...
class MRDwiAdcProcessingStrategy(ProcessingStrategy):
    ...
    def process(self, study_path: pathlib.Path, *args, **kwargs):
        ...
        for series_folder in tqdm(series_folder_list, desc=f"{study_path.name}"):
            dicom_list = [dcmread(str(x), force=True) for x in series_folder.iterdir()]
            for _dicom in dicom_list:
                pass
