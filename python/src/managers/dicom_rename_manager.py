...
from ..core.exceptions import ProcessingError, DicomParsingError
from ..core.types import DicomDataset, ExecutorType
from ..core.logging import get_logger
from ..processing.factory import ProcessingStrategyFactory
from ..processing.base import ModalityProcessingStrategy, MRAcquisitionTypeProcessingStrategy
...
class DicomRenameManager(BaseManager):
    ...
    def __init__(self, input_path: Union[str, Path], output_path: Union[str, Path]):
        ...
        self.mr_acquisition_type_processing_strategy = MRAcquisitionTypeProcessingStrategy()
        self.processing_strategy_list = ProcessingStrategyFactory.create_all_mr_strategies()
    ...
    def determine_series_name(self, dicom_ds: DicomDataset) -> str:
        ...
        try:
            ...
            mr_acquisition_type_enum = self.mr_acquisition_type_processing_strategy.process(dicom_ds)
    ...
    def process_single_dicom_file(self, dicom_file_path: Path) -> bool:
        ...
        try:
            ...
            self._logger.log_error(
                DicomParsingError(f"DICOM 檔案解析失敗: {str(e)}", dicom_file=str(dicom_file_path)),
                context={"file": str(dicom_file_path)}
            )
            return False
        except Exception as e:
            self._logger.log_error(e, context={"file": str(dicom_file_path), "operation": "process_single_dicom_file"})
            return False
    ...
    def run(self, executor: ExecutorType = None) -> None:
        ...
        try:
            ...
            if executor:
                ...
                with self._create_executor(Config.DEFAULT_WORKER_COUNT) as rename_executor:
    ...
    def _process_with_executor(self, dicom_files: List[Path], executor: ThreadPoolExecutor) -> None:
        """使用執行器處理 DICOM 檔案"""
        futures = []
    ...
    def get_processing_statistics(self) -> dict:
        ...
        try:
            ...
            output_dicom_count = len(list(self.output_path.rglob('*.dcm'))) if self.output_path.exists() else 0
            
            return {
                'input_dicom_files': input_dicom_count,
                'output_dicom_files': output_dicom_count,
                'processing_rate': output_dicom_count / input_dicom_count if input_dicom_count > 0 else 0,
