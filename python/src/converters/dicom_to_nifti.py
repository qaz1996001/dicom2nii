...
class DicomToNiftiConverter(BaseConverter):
    ...
    def convert(self, executor: ExecutorType = None) -> None:
        ...
        try:
            ...
            for study_path in study_list:
                ...
                # 獲取序列列表
                series_list = [
                    series_path for series_path in study_path.iterdir()
                ...
                for series_path in series_list:
                    ...
                    # 提交轉換任務
                    if executor:
                        future = executor.submit(self._convert_series, output_series_path, series_path)
        ...
        try:
            ...
            if completed_process.returncode != 0:
                raise ConversionError(
                    f"dcm2niix 執行失敗: {completed_process.stderr}"
                )
    ...
    def _handle_output_files(self, result_str: str, output_series_path: Path, expected_output_path: Path) -> None:
        """處理輸出檔案的重新命名"""
        ...
        if dcm2niix_output_path.name != expected_output_path.name:
            try:
                ...
                # 重新命名對應的 JSON 檔案
                dcm2niix_json_path = Path(str(dcm2niix_output_path).replace('.nii.gz', '.json'))
                expected_json_path = Path(str(expected_output_path).replace('.nii.gz', '.json'))
    ...
    def get_conversion_statistics(self) -> dict:
        ...
        try:
            ...
            return {
                ...
                'conversion_rate': total_nifti_files / total_dicom_files if total_dicom_files > 0 else 0
            }
        except Exception as e:
