...
def validate_dicom_processing_params(
    input_path: str, output_path: str, worker_count: int
) -> None:
    ...
    validate_required_params(
        input_path=input_path, output_path=output_path, worker_count=worker_count
    )
...
def validate_upload_config(web_url: str, file_paths: list[str]) -> None:
    ...
    if not url_pattern.match(web_url):
        raise ValidationError(
            f"無效的 URL 格式: {web_url}", field_name="web_url", field_value=web_url
        )
