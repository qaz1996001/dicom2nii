...
def extract_dicom_info(dicom_ds: DicomDataset) -> dict[str, Any]:
    ...
    try:
        ...
        # 處理時間格式
        study_time_value = study_time.value.split('.')[0] if study_time and '.' in study_time.value else study_time.value if study_time else None
        series_time_value = series_time.value.split('.')[0] if series_time and '.' in series_time.value else series_time.value if series_time else None
...
def validate_dicom_tags(dicom_ds: DicomDataset, required_tags: list[tuple[int, int]]) -> bool:
...
def get_image_orientation_with_reformatted(dicom_ds: DicomDataset,
                                         image_orientation_strategy) -> Any:
    """獲取包含重新格式化資訊的影像方向"""
    try:
        # 獲取基本影像方向
        ...
        if image_type and len(image_type.value) > 2 and image_type.value[2] == 'REFORMATTED':
            # 根據原始方向返回重新格式化的方向
            orientation_map = {
                'AXI': 'AXIr',
                'SAG': 'SAGr',
            ...
            if hasattr(image_orientation, 'value') and image_orientation.value in orientation_map:
                # 這裡需要返回對應的重新格式化枚舉值
                from ..core.enums import ImageOrientationEnum
                return getattr(ImageOrientationEnum, orientation_map[image_orientation.value])
...
def clean_dicom_dataset(dicom_ds: DicomDataset, exclude_tags: dict[str, str]) -> DicomDataset:
