...
def generate_study_report(data_path: PathLike, output_format: str = 'both') -> pd.DataFrame:
    """生成檢查報告"""
    try:
...
def generate_series_report(data_path: PathLike, output_format: str = 'both') -> pd.DataFrame:
    """生成序列報告"""
    try:
        data_path = Path(data_path)
        series_data = []
...
def generate_nifti_report(data_path: PathLike, output_format: str = 'excel') -> pd.DataFrame:
    """生成 NIfTI 檔案報告"""
    try:
        data_path = Path(data_path)
        nifti_data = []
...
def create_processing_summary(processed_studies: list[str],
    ...
    try:
        summary = {
            ...
            'success_rate': len(processed_studies) / (len(processed_studies) + len(failed_studies)) * 100 if (processed_studies or failed_studies) else 0,
